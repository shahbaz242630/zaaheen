...
## 8.38 · ADR-104 / ADR-SEC-022 amendment 12 (2026-09-21, session 50) — decisions for S3 step 4d-1 (what the screens stand on)

Step 4d is split the way step 4 was: **4d-1** the desktop plumbing the screens need · **4d-2** the screens (lock screen, onboarding sign-in step, account panel, banners) · **4d-3** the keeper side (the start-up refresh, and the agent-facing trial and payment warnings of §6). The vault key's Local-persistence fix (HANDOFF §3 item 6) is its own step after 4d, because it moves the key that unlocks every memory. BRD §11 was re-read in full before this step (§11.15); the parts that bear on it are §11.5.1 (encryption at rest, "no exceptions"), §11.7.1 (every Tauri argument validated), §11.7.2 (no internal text reaches the UI) and §11.9.1 (login and logout are logged).

### ADR-SEC-027 — 2026-09-21 — the signed-in address is kept in Credential Manager, beside the token (FOUNDER-LOCKED)

- **Context.** §8.37 left it open: the address is known only at sign-in, so from the second app open §3's *"Signed in as <email> — not you? Sign out"* has no address to show.
- **Decision (founder, 2026-09-21: "yes A").** At sign-in, after the refresh token is stored and the marker written, the account stores the address as a second credential of its **own** store: service `com.zaaheen.account`, user `signed-in-email`, **Local** persistence, value `<sub>\n<email>`. It is shown only when its `sub` equals the marker's, and only when both pass the checks `userinfo` applies on the way in (non-empty, bounded, no control characters). Every path that clears the marker deletes it: sign-out, the local sign-out after a dead grant or 30 days unused, and "Delete everything".
- **Reasoning.** BRD §11.5.1: *"All data on disk is encrypted. No exceptions."* A file in the account folder would be the one plaintext identity on disk; Credential Manager encrypts under the person's own logon, which is where the refresh token already lives. Binding the address to the `sub` means an address left behind by an earlier person can never be shown under a new one, even when a delete failed.
- **Alternatives considered.** (a) A plaintext file in the account folder — rejected by §11.5.1. (b) A `userinfo` call when the panel opens — rejected: the only way to an access token is to rotate the refresh token under the lock (ADR-SEC-025), the most fragile step in this design, and doing that for a label multiplies the risk; offline it shows nothing, which is when somebody most needs to know who is signed in.
- **Trade-off.** Somebody who changes their address in their Zaaheen account sees the old one here until they next sign in. It is a label, not an entitlement. Accepted.
- **Failure rule.** A refused write is logged and does **not** fail the sign-in — the sign-in is the token, not the label — and the panel then says "Signed in" with no address. A refused read is the same.
- **§4 amended.** The account's local state is its four files **and two credentials** (the refresh token and the address).

### ADR-SEC-028 — 2026-09-21 — one account per desktop process

- **Context (found reading the code for this step).** The desktop built the account **twice**: `guard::build()` → `build_check` for the lock, and `main.rs` → `build_account_ops` for the buttons. §8.27 keeps a rotated refresh token the credential store refused *in memory*, because it is the only live one, and accepts one residual: *"if that process exits, or another Zaaheen process refreshes, while the store is still refusing … that refresh signs the user out."* Two copies inside one process turned that cross-process residual into a same-process one: the copy that rotated holds the live token, the other copy's next refresh replays the spent stored one, gets `invalid_grant` twice, and signs the person out. The file lock serialises the two copies; it cannot share memory between them. And they refresh independently — the lock's copy on a refused call, the buttons' copy on "I've paid", checkout polling and the daily timer.
- **Decision.** `vault_app::account::DesktopAccount` is built from **one** `Arc<Account>` and hands out the check and the operations over it; its constructor takes a single account, so two cannot be mixed. `guard::build()` stays the only way a binary obtains an `Entitlement`, and now also returns the `AccountSlot` the account commands read — both views of one account, or neither. `build_account_ops` is removed, so there is no second way to build it.
- **Consequences.** The residual is back to §8.27's accepted cross-process case (the keeper and the desktop are still two processes). The lock and the buttons can no longer disagree about whether an account exists: a build without account settings has neither; a build whose account could not be prepared has a locked guard (ADR-SEC-023) and an empty slot, whose commands answer the stable `account_unavailable` ("close Zaaheen and open it again") instead of Tauri's raw *"state not managed"* text (BRD §11.7.2).
- **A correction to ADR-SEC-023's wording.** It says that on a desktop whose check could not be built *"the person still reaches the lock screen, can still download their memories, and can still sign in and subscribe."* The last part was never reliable — the buttons' account was built from the same settings, folder and credential store that had just failed — and with one account it is plainly not so. What holds is the part that matters: the lock screen and the export. Signing in and subscribing need the app reopened, and the screen says so.

### The screen asks the gate; it never infers

- **Context.** `AccountView.state` is the **lease's** state (`trial`, `active`, …), not the verdict. A trial past its deadline on a computer with no internet still reads `trial` with 0 days left, while the gate says "could not confirm" (§8.27's mapping). A lock screen inferred from the view would disagree with the lock itself.
- **Decision.** One new command, `account_access`, answers `{ "sign_in": bool, "locked": null | "<locked_* code>" }`. `locked` comes from `Entitlement::lock_code()`, which shares `require()`'s single path — one ask of the check, refresh-then-decide — so the lock screen shows exactly when a gated command would be refused. `sign_in` is `false` only in a build with no account settings, so a developer build never shows a sign-in screen. **Step 4d-2 (not built in 4d-1)** is to make the frontend treat any `locked_*` rejection from a gated command as the same signal, so that a subscription that lapses while the app is open reaches the lock screen on the next action. Until 4d-2 there is no lock screen: 4d-1 only builds what it will call.
- **Not a widening of §6.4.** It fills the **account** slot, as the five account commands did (§8.37). It takes the guard and nothing else, so ADR-SEC-024's argument holds and `no_account_command_receives_the_vault` still covers it. `the_open_list_is_exactly_the_locked_allowlist` is updated, citing this paragraph.
- **It records a use when entitled**, as every served call does: opening the app is using the vault (§4). It is asked when the app opens and after an account action, never on a timer.
- **An obligation on 4d-2, because nothing in 4d-1 enforces it** (the independent review, round 2): `record_use` throttles the writes, but a frontend that *polled* `account_access` would keep `last_active_anchor` current for as long as the window stayed open, and §4's 30-days-unused sign-out would never fire. 4d-2 must pin "no `account_access` inside a timer" with a `frontend_contract` test, and its review must check it.

### Two obligations that were marked done, and were not

Retracted with the evidence that falsified them:
- **§8.37 says** *"`spawn_routine_refresh` refreshes on desktop open and then daily"*, and HANDOFF said `Trigger::Routine` *"finally has a production caller"*. **Falsified by** `grep -rn spawn_routine_refresh crates/` (2026-09-21): defined and unit-tested, called from nowhere. Step 4d-1 wires it. It also diverged from §4, which refreshes at open **only** when `stale_at_start` holds (the lease is more than a day old, or a deadline is within three days) and otherwise leaves it to the daily timer. The wiring follows §4, so an ordinary open does not rotate the token.
- **The keeper-start refresh** (§4: *"At keeper start and desktop open"*) has no caller either: `stale_at_start` is used nowhere outside its own tests. Step 4d-3 owns it, with the rest of the keeper side.

### "Delete everything" signs out (§7, not built until now)

§7: *"Delete everything (erasure + local sign-out + revoke; dialog says the subscription continues until cancelled, with the portal link)"*. The erasure runs first and alone decides the result shown. Only once the key has been destroyed does the account sign out and revoke, best effort: a failure there is logged and never turns a completed erasure into a reported failure. If the erasure fails, nothing else changes and the person stays signed in, because erasure *"either succeeds at the step that matters or does nothing at all"* (ADR-SEC-008). The dialog's wording is step 4d-2's.

### The clock notice (§4, not built until now)

§4: *"if `|client_now − server_now| > 24 h`, the desktop shows 'Your computer's clock is wrong. Set it to update automatically.' as information only"*. `AccountView` gains `clock_wrong`, read from the lease on disk with `clock_looks_wrong`, so the notice survives a restart rather than showing once at sign-in. The view's wire allowlist grows from four fields to five, on purpose.

### Evidence (session 50, 2026-09-21)

- **Crate-scoped runs, all green:** `vault-account` 240 (18 new, run failing first: 8 failed / 232 passed against inert stand-ins, each for the reason it names, then green; one of the new tests runs against the real Windows Credential Manager), `vault-app --lib` 258, `vault-tauri --lib` 87, `frontend_contract` 20 (which also compiles `main.rs`). **The full DoD gates were not run**: by the founder's direction they run once, from a wiped `target\debug`, after 4d-2, 4d-3, the vault key's Local persistence and the user-chosen vault location.
- **15 planted bugs, 15 caught** — after one test was replaced. d1-08 (a leased computer refreshing at every open) was first **MISSED**: a source test checked that the decision's text *contained* `stale_at_start(assessment)`, and the plant appended `|| true`, which still contains it. The decision now takes a small `AtOpen` view so it is tested with real `Assessment`s (`a_fresh_lease_is_not_refreshed_at_open`, `a_stale_lease_is_refreshed_at_open`), and the same plant is caught. d1-15 proves `no_command_module_builds_a_guard_of_its_own` still sees a real `guard::build()` call now that it strips comments — it had fired on `account.rs`'s own documentation, the fourth time a source test read prose as code.
- **Independent review:** one real finding, a sentence in this amendment describing the lock screen as already reacting to a lapse (it is 4d-2's; reworded). Round 2 read the unchanged files and confirmed no deadlock between in-process refreshes (each lock is a fresh handle with a bounded wait), and raised the `account_access` polling obligation recorded above.

---

## 8.39 · ADR-104 / ADR-SEC-022 amendment 13 (2026-09-21, session 51) — decisions for S3 step 4d-2 (the screens)

Everything behind the screens was built in 4b–4d-1; 4d-2 is the part a person sees. **No production Rust changed**: every command the screens call already existed, so there is no new command, permission or capability, and ADR-SEC-024's "never give them the vault" is untouched. The step is `dist/app.js`, `dist/index.html`, `dist/styles.css` and 14 new guards in `tests/frontend_contract.rs`. BRD §11 was re-read in full before the step (§11.15); the parts that bear on it are §11.2 SP-4 (fail securely: an unknown lock code shows the try-again screen, never an open app), §11.4.1 (see the divergence below), §11.7.2 (only fixed lines reach the page) and §11.12's `vault-tauri` checklist (no markup built from account data; CSP unchanged).

### Two founder decisions (2026-09-21, session 51)

- **The support address is `customerservice@zaaheen.com`** (*"we already have customerservice@Zaaheen.com set up"*; it is live in Outlook and lands in the Customer Service shared mailbox). It is on the lock screen and, beside the activity-record export, in Settings. Public by design; nothing else about the mailboxes goes in this repo.
- **The lock screen's words** (*"yes partner go with that wording"*). Five screens, by the lock's reason; on every one, **Download my memories** with "Save everything you've kept as one file. It's yours, whether or not you subscribe." and "Need help? customerservice@zaaheen.com":
  1. *signed out* — "Sign in to open your memories." · "Your memories are on this computer, encrypted, just as you left them. Sign in to your Zaaheen account to open them. New to Zaaheen? Create an account on the next page. Your 30-day free trial starts straight away, and no card is needed." · **Sign in** · "This opens your web browser. Signing in never shares your memories with us. They stay locked on this computer." · while waiting: "Finish signing in in your browser, then come back here."
  2. *trial ended* — "Your free trial has ended." · "Subscribe to keep using your memories. Nothing has been deleted. Everything you kept is still here on this computer." · **$5 a month** · **$48 a year (save $12)** · "Payment opens in your browser. Tax is added at checkout where it applies. Cancel anytime." · then "Finish paying in your browser. Zaaheen unlocks by itself as soon as your payment comes through." + **I've paid** · "We haven't seen your payment yet. It can take a minute, so try again shortly." · "Signed in as … Not you? Sign out"
  3. *subscription ended* — as 2, with "Your subscription has ended." and "Subscribe again to keep using your memories. …"
  4. *could not confirm* — "We couldn't confirm your subscription." · "Zaaheen couldn't reach us to check your subscription. Check your internet connection, then try again. Only your subscription is checked, never your memories." · **Try again** · "Still couldn't connect. Check your internet connection and try again in a moment."
  5. *needs reopening* — "Zaaheen couldn't open your account on this computer." · "Closing Zaaheen and opening it again usually fixes this. If it keeps happening, email us at the address below." · **Close Zaaheen**

  Pinned word for word by `the_lock_screen_says_what_the_founder_approved`. The prices sit on the buttons, so nobody meets the price for the first time on the payment page; "save $12" is 12 × $5 − $48. The sixth code, `locked_unlocking`, reaches only a lock-mode keeper, never this app's own check; it is mapped anyway, with §8.26 §6's own words ("Zaaheen is unlocking. Try again in a moment."), so the mapping stays total.

### How the screens stand on the lock

- **The lock is asked in one place, `askAccess`, and never on a timer** — §8.38's obligation, discharged. Its callers are listed with their reasons in `ACCESS_ASKED_FROM`: the app opens; after sign-in, sign-out, "I've paid" and "Try again"; once after a checkout's wait. `account_access_is_never_asked_on_a_timer` checks five things: one ask; only the listed callers; nothing that reaches it, directly or through other functions, contains a timer or a loop or registers any listener but a click; no timer is handed any of them; and wherever one is handed over as a callback, it is to a button's `click`. The **checkout wait** (`waitForPayment`, every 10 s for 10 min, §4) therefore only refreshes and returns, and `afterCheckout` asks the lock once, afterwards.
- **"Needs reopening" is told apart from "check your connection" by the account view, never by guessing.** Both arrive as `locked_cannot_confirm`. The view says `signed_in: false, state: "cannot_confirm"` only when there was no account to read at all — the empty slot of ADR-SEC-028, or an account folder that could not be read — and reopening is what fixes that. Signed in but offline (or no lease yet) gets the try-again screen. An unknown code also falls back to try-again (SP-4).
- **A refused command anywhere sends the person to the lock screen** (§8.38). The central `invoke` wrapper, which already turned a lock code into its line, now also calls `showLock`, so a subscription that lapses while the app is open reaches the lock screen on the next action, whichever button it was.
- **Nothing gated starts before the lock answers.** The recall-engine download, its readiness poll, the maintenance-engine download and the missed-run catch-up all call gated commands. They used to start the moment the window opened, so a fresh install's first download was refused before anybody could sign in. They now start from one place, `startEntitledWork`, once, on the branch where the lock said yes. Pinned by `nothing_gated_starts_before_the_lock_has_answered`.
- **A returning computer shows nothing until the lock answers**, so a locked one never flashes its memories. The answer is a file read when all is well; if it takes longer than 400 ms (a refusal refreshes first, up to five seconds, §4), the lock screen says "Checking your subscription…" with its foot already showing.
- **Onboarding signs in straight after the welcome**, before the recall-engine download and the first memory, and only on a computer nobody is signed in on: five steps (`ONBOARDING_WITH_SIGN_IN`) instead of four. A new install whose lock is shut for another reason (a trial that ended on an earlier install, no internet) meets the lock screen at "Begin set up", and goes on into setup once it opens. A build without account settings keeps today's four steps and shows no account surface at all (§8.38: "a developer build never shows a sign-in screen").

### The rest of 4d-2

- **Account panel** (Settings → Your account): "Signed in as … Not you? Sign out", the state (with the days left in a trial), the two plans for a trial, **Manage subscription** for a subscriber and for a failed payment, and the same checkout wait with **I've paid**. Sign-out asks first: "Your memories stay on this computer, encrypted. You'll need to sign in again to use them."
- **Banners** on the home screen, information only: the trial's last seven days (`days_left <= 7`, §6's "from day 23"), a failed payment with **Update your card**, and §4's clock notice word for word. Read from `account_status` whenever the home screen or Settings is drawn: a disk read, with no network, no lock asked and no use recorded.
- **Download my memories** on the lock screen (in its foot, which no reason changes) and in Settings → Your memories. Save dialog, `zaaheen-memories-<date>.md`, "Saved 42 memories."
- **Delete everything** now tells somebody paying, "Deleting your memories does not cancel your subscription. It continues until you cancel it.", with a **Manage subscription** button (§7's "with the portal link" — a button, because no address crosses into the page, ADR-SEC-026). The erasure's own message is unchanged: `erase_everything` does not report whether its best-effort sign-out succeeded, so the page does not claim it.
- **The signed-in address is only ever text** (`textContent`), pinned by `the_signed_in_address_is_only_ever_text`: a function that reads it may not write markup at all.
- **Account and export lines are now whole sentences**, since they stand on their own under a button ("Signing in didn't finish. Try again."). Lines are thrown as strings, the way Tauri rejects, so a call site's `${err}` no longer reads "Error: …".
- **One check at a time** for "I've paid", "Try again", Subscribe and sign-in: an overlapping click does nothing, so a double click cannot ask the lock twice.

### A divergence, recorded rather than decided

**BRD §11.4.1** lists *"Exporting full vault"* among the operations that get **multi-factor by default**. The export (S4, built in 4c) has none, and 4d-2 is what puts a button on it. It is not a new exposure: whoever can press the button is at the same unlocked Windows session that can already read every memory in the app, and the lock is a business control, not a security boundary (§8.26 §6.7). A second factor would need either the account — which a locked person may not be able to use, the very case the export exists for — or a vault passphrase V0.2 does not have. **Left as it is, and raised with the founder in HANDOFF §3.**

### Evidence (session 51, 2026-09-21)

- **Crate-scoped runs, all green:** `vault-tauri --lib` 87, `frontend_contract` 34 (20 before; 14 new), each run confirmed to recompile the crate. **The full DoD gates were not run**, by the founder's direction (one fresh run after 4d-3, the vault key and the vault location).
- **Run failing first:** against the pre-4d-2 screens, 13 of the 14 new guards failed, each for the reason it names — among them `init() calls startEngineFetch, a gated command, before the lock has answered`, the fresh-install bug. The fourteenth, `nothing_the_desktop_shows_names_a_vendor`, pins a property that already held and was proven by planting.
- **19 planted bugs, 19 caught**, each by the guard aimed at it and nothing else: the lock asked from a timer, from a new self-rescheduling function, from inside the checkout wait, from a loop two calls away, and from a `focus` listener, as a callback or registered inside a function that reaches it; the checkout wait with no end; the wrapper not sending a refusal to the lock screen; gated work started in `init`, or started on the locked branch as well; the address written as markup; the export hidden on the subscribe screens; "save $12" changed; the sign-in step moved after the first gated step; the trial banner a day late; every could-not-confirm screen shown as "reopen"; a vendor name; the account panel in a build without sign-in; the Delete dialog no longer saying the subscription continues. **One guard was strengthened before its plant ran:** designing the "started on the locked branch as well" plant showed the test checked the open branch but not that it was the only one.
- **Executed, not only read:** `dist/` was driven in a browser with a stand-in for the Tauri bridge that mirrors the commands (gated calls refused with the lock code while locked). Thirteen flows behaved as designed: a fresh install (only `account_access` before sign-in; STEP 2 OF 5; the downloads start after it), trial ended through checkout, "I've paid" before and after the payment, and the unlock (the lock asked once after the wait), could not confirm and Try again, needs reopening and Close, a returning signed-out computer, a lapse mid-session reaching the lock screen on the next search, the trial banner at 5 days and the account panel, payment failed with the clock notice, a subscriber's Delete dialog, a developer build (no account surface, no `account_status` call), a hostile address shown as text, the export, and twelve idle seconds with no second ask. A double click on "I've paid" with a realistic 500 ms refresh made one refresh and one ask.
- **Independent review:** two findings, both fixed. (1) The timer guard only looked at timer arguments, so `window.addEventListener("focus", askAccess)` — a plausible "refresh on focus" feature — would have asked the lock on every focus with every guard green. Part 5 was added, and plants for both shapes were caught. (2) This amendment's draft said the §11.4.1 divergence was raised in HANDOFF §3 before it was; it is there now. Also noted, below its threshold: "I've paid" had no guard against an overlapping click, unlike "Try again"; one was added and checked in the browser.

---

## 8.40 · ADR-104 / ADR-SEC-022 amendment 14 (2026-09-21, session 51) — decisions for S3 step 4d-3 (the keeper side)

Two things §8.26 asks of the keeper that nothing did yet: §6's *"payment failed (still entitled; `health.warnings` + desktop banner)"* and *"Trial reminders: desktop banner from day 23; `health.warnings` in the last 5 days"*; and §4's *"At keeper start and desktop open: refresh if the lease is older than 24 h or any deadline is within 3 days. Otherwise a jittered daily timer while running (never triggered by tool activity)"*, whose keeper half had no caller (§8.38). BRD §11 was re-read in full this session (§11.15, for 4d-2); the parts that bear on this step are §11.2 SP-4 (a notice never accompanies a refusal), §11.7.2 (fixed text, no user data, to the agent) and §11.4.4's generic-error rule, which is untouched: a notice is added only to a served call.

### Founder decisions (2026-09-21, session 51: "yes all good")

- **The words the AI apps relay.** Payment failed, as locked in session 43: *"Your last Zaaheen payment didn't go through. Open the Zaaheen app to update your card."* Trial ending, new: *"Your Zaaheen free trial ends in 3 days. Open the Zaaheen app and choose Subscribe to keep using your memories."* — "in less than a day" and "in 1 day" at the end; the second sentence is session 43's locked phrase.
- **Once per conversation.** The notice is on every `memory_read` while it applies; `memory_read`'s description tells the agent to mention it once per conversation, in one sentence, then answer normally.

### ADR-054 Contract 2, amendment 3 — two account codes in `health.warnings`

- **Decision.** `WarningCode` gains `SUBSCRIPTION_TRIAL_ENDING` (severity `info`) and `SUBSCRIPTION_PAYMENT_FAILED` (severity `warn`). `detail` is the first sentence above and `recovery_hint` the second, so the pair reads as the approved words. They are added by the MCP layer from the account, never by the read pipeline, and **they never change `health.status`**: `status` goes on describing the vault's data (REPORT age, topic names, the clock), which is what an agent uses it for. `memory_read`'s description says both things.
- **Why a warning, not a new field.** ADR-052's rule is that the vault returns structured facts and the agent speaks. `health.warnings` is already the channel the description tells every agent to read and relay when material; a new field would be one more thing each of five apps has to be taught.
- **Only `memory_read`.** It is the primary tool (ADR-071) and the only one with `health`. `memory_search` and the write tools carry no notice.

### How the notice travels — the lock's answer is not reshaped

- **`EntitlementCheck::check_with_notice()`** returns the verdict and, only when it is `Entitled`, an `AccountNotice` (`TrialEnding { days_left }` or `PaymentFailed`). Its default returns no notice, so every existing check — lock mode, the desktop's guard, the tests' stand-ins — is unchanged. `Verdict` keeps its shape: it has 48 uses across five crates, and a notice is not part of whether a call is served.
- **The gate carries it in the request.** On an entitled tool call, `EntitledService` puts the notice into the request's own `extensions` before passing the call on, and `memory_read` reads it there (rmcp 2.2.0: `RequestContext.extensions`, read by a tool through the `Extensions` extractor — checked in the crate's source, `handler/server/common.rs`). The extensions exist only inside the process, so no client can supply or forge a notice; a locked call never reaches the server, so it cannot carry one. The daemon, which asks the check itself, does the same.
- **Computed from the same reading that decided the call.** `AccountCheck` now reads, with the verdict, the lease's state and its remaining time (`AccountState::Leased` carries the `Assessment` and whether the lease is a trial). Payment failed → `PaymentFailed`. A trial with less than five days left (`remaining < 5 × DAY`, whole days rounded down: 4…0, days 26–30 of 30 — the same counting as the desktop's "from day 23", `days_left <= 7`) → `TrialEnding`. After a refresh-then-decide, the notice comes from the reading after the refresh.

### The routine refresh, shared by the keeper and the desktop

- **One rule, one place.** `vault_app::entitlement::refresh_at_start_then_daily` runs §4's routine for any account: at start, refresh when nobody-has-a-lease-yet or the lease is stale (older than a day, or a deadline within three days); then a jittered daily tick. The desktop's 4d-1 version (`AccountOps::refresh_at_open_then_daily`, with its private `wants_refresh_at_open` and `daily_period`) now calls it, so the keeper and the desktop cannot drift apart; their tests move with it.
- **The daily tick refreshes only when the lease is stale.** 4d-1's desktop tick refreshed unconditionally. With the keeper running the same routine, the two processes share one lease file: whichever refreshes first makes it fresh and the other's tick then skips, so a computer with both open still asks the account service about once a day (the Worker's free quota, §8.26 §4). A process alone sees a lease at least a day old at every tick, so it refreshes as before.
- **The keeper starts it on the full path only**, in its own task, before the models load: neither the start nor any call waits on the network. Lock mode needs none — its start already refreshed (refresh-then-decide), and every call it answers asks again. It records no use (a tick is not somebody using their vault, §8.35). Direct mode (`--direct`, a developer opt-out that bypasses the keeper) carries the notices through the same gate but has no routine refresh; recorded, not built.

### Evidence (session 51, 2026-09-21)

- **Crate-scoped runs, all green:** `vault-mcp` — `entitlement_gate` 28 (9 new), `daemon_auth` 7 (1 new), lib 6; `vault-app --lib` 268 (258 at 4d-1; the desktop's six at-open and timer tests moved into `entitlement/tests.rs` with the functions); `vault-cli --bin zaaheen` 41 (1 new); `vault-tauri --lib` 87 and `frontend_contract` 34, recompiled against the shared routine. **The full DoD gates were not run**, by the founder's direction.
- **Run failing first:** against inert stand-ins, 12 tests failed across the three crates, each for the reason it names — the three notices that never arrived and the description that did not explain the codes (`vault-mcp`); the notice never worked out, the staleness rule never saying yes, and the routine never refreshing (`vault-app`, 7); the keeper not spawning the routine (`vault-cli`). The other new tests passed from the start because they pin properties that already held — the words, a refusal carrying nothing, only `memory_read` carrying a notice, an AI app unable to supply one, the trait's default adding nothing, a fresh lease left alone, the timer's window — and the pins that matter among them were planted.
- **9 planted bugs, 9 caught**, each by the test aimed at it: a refusal carrying the notice's words; `memory_read` downgrading `status` when a notice is added; the trial window at `<=` five days; a paid subscription treated as a trial; the notice taken from the stale reading instead of the one after the refresh; a signed-out computer refreshed at start; the routine refresh recording a use; the daily tick refreshing a fresh lease; the keeper starting the routine above the lock-mode branch. **Held by structure, not planted:** the daemon returns on a refusal before it touches the notice, and `Gate::verdict_with_notice` drops one on a refusal as well, so neither layer alone is observable.
- **Independent review:** no finding at its threshold. It checked in rmcp 2.2.0's source that every incoming request starts with fresh, empty extensions (`service.rs`), so nothing on the wire — `_meta`, arguments — can populate them; traced the notice from the gate through a relay to the AI app; and confirmed nothing new records a use. Its one note below threshold — the keeper's wiring test checked "spawn" and the routine as two separate substrings — was taken: the test now requires the one connected call, and it was re-run green.

**§8.40 addendum (session 52) — the wire version.** `memory_read`'s new description changed the MCP tool contract, and 4d-3's own runs did not include `vault-app --test keeper_end_to_end`, whose `the_tool_contract_is_pinned_to_the_wire_version` caught it in session 52. As that test directs: `vault_app::keeper::handshake::WIRE` 2 → **3**, and the pinned contract hash is now `7a941150d4fe95809f1a0847607b607d3ecc79caabfad2e2f645719e1af4584d` (computed on an LF working copy, the same bytes CI's Linux and macOS legs hash). Effect: a relay from an older build meeting this keeper (or the reverse) answers "Zaaheen was updated; restart every app that uses Zaaheen" instead of serving a contract it does not know. Rides with 4d-3's commit.

## 8.41 · ADR-104 amendment 15 (2026-09-22, session 54) — "Create an account" beside "Sign in" (FOUNDER-APPROVED)

**Founder, looking at the setup's sign-in step in the browser preview:** *"create an account on next page ? but the next page is not showing create account .. we should have both sign in or sign up links here.. if user is existing user .. clicks sign in .. land on sign in page .. once signed in land back at app .. if new user clicks sign up.. lands at sign up page ... once successfull .. lands back at app .."*; shown the plan and the words: *"yes please build the buttons and wording is good"*.

**What changes.** The setup's sign-in step and the signed-out lock screen get two buttons, **Create an account** and **Sign in**. Both run the one sign-in of §8.26 §3 — the same `PendingSignIn` (loopback listener, PKCE, `state`, the `iss` check), the same callback, the same exchange. Only **the first page the browser shows** differs:
- **Sign in** opens the authorize URL, as before; signed out, the account service shows its hosted sign-in page.
- **Create an account** opens the account service's hosted **sign-up** page with `redirect_url` set to that same authorize URL. After sign-up the hosted page follows `redirect_url` to the authorize endpoint, now signed in, which redirects to the loopback exactly as a sign-in does.

**Measured before building (session 54, read-only, against the development instance):** the authorize endpoint ignores OpenID's `prompt=create` (and `prompt=signup`): with or without it, a signed-out browser is sent to the hosted **sign-in** page with `redirect_url=<issuer>/oauth/authorize-with-immediate-redirect?…`. The hosted pages take `redirect_url` for exactly this purpose, so the sign-up page with our authorize URL as its `redirect_url` is the documented way round. **Still to be seen by a person** (the service blocks automated sign-up, §8.31's note): one real sign-up through the new button landing back in the app — at the next live test.

**Where the sign-up page is.** Derived from the compiled issuer by the account service's fixed naming — a development issuer `https://<name>.clerk.accounts.dev` has its hosted pages at `https://<name>.accounts.dev`, a production issuer `https://clerk.<domain>` at `https://accounts.<domain>` — so no new build setting is needed (§8.33's settings stay seven). An issuer of any other shape has **no** derived page, and "Create an account" then opens the ordinary sign-in page, whose own "Sign up" link carries the same `redirect_url`: honest, and still back in the app.

**Security (BRD §11.4).** Nothing new is trusted. The sign-up URL is built in this process from the compiled issuer — never from anything the account service or the webview supplied — and passes `ExternalLink`'s gate like every link (§8.37). Its one parameter is our own authorize URL, whose values (`client_id`, `redirect_uri`, `state`, `code_challenge`) the browser sees on the ordinary path anyway; the PKCE verifier never leaves the process. The webview chooses only **which** of the two pages, as a closed enum (`sign_in` | `sign_up`), and no Tauri command takes a URL (§8.37's rule holds). The callback's checks are unchanged, so a sign-up that went anywhere else simply never completes.

**Words** (founder-approved with the plan, no long dashes, founder session 54): the setup's step — *"Create your Zaaheen account, or sign in."* / *"Your 30-day free trial starts when you create your account, and no card is needed."* / **Create an account** (primary) · **Sign in**; the small print unchanged. The lock screen's signed-out text drops "on the next page": *"… New to Zaaheen? Create an account, and your 30-day free trial starts straight away. No card is needed."* — **Sign in** (primary, a returning computer) · **Create an account**.

## 8.42 · ADR-104 amendment 16 (2026-09-22, session 55) — the founder's walk-through: the two buttons alike, and the lock screen's download only where there is something to take (FOUNDER-APPROVED)

**1. "Create an account" and "Sign in" look alike.** Founder, on the setup's step: *"both are not balanced.. and different color... can we have sign up and sign in both in same color like black .. this will keep them same size"*. Both are now the dark pill, side by side at one width (each column as wide as the longer label), on the setup's step and on the signed-out lock screen alike. §8.41's "(primary)" no longer describes a difference in look; the order stays (Create an account first on the setup's step, Sign in first on the lock screen). Labels unchanged. Guard: `both_pages_are_offered_and_run_the_one_sign_in`.

**2. The lock screen offers "Download my memories" whenever this computer has memories, and hides it where there is nothing to take.** Founder, seeing the download on a signed-out lock screen during a new install: *"this should be in the settings page"*; shown why it is also on the lock screen (Settings cannot be opened while locked, so it is the only way out for somebody whose trial has ended; BRD §1.6 amendment 1, *"It's yours, whether or not you subscribe."*), and the recommendation to keep it where there are memories and hide it where there are none: *"ok perfect but we also need it in the settings page"* (it is there, unchanged, and shown to him).
- **The rule is the count, never the reason for the lock** (§8.39's "whatever the reason" still holds; `renderLock` does not touch the download). The count is `get_settings_info`'s `memory_count`, a command already on the locked allowlist (§8.26 §6.4's settings slot), so nothing is widened and no Rust changes.
- **Hidden only on a count that reads exactly zero.** A count that cannot be read, or a reply without one, leaves the download shown; it starts shown in the markup, so nothing has to go right for it to appear. Hiding it from somebody who has memories is the failure that matters; showing it to somebody with none costs a button.
- **Asked each time the lock screen is entered**, not per lock answer, so the checkout's ten-second polling (§8.26 §4) does not repeat it (`get_settings_info` writes an audit entry each call).
- Why not "has this computer finished its setup" (`mv_onboarded`, webview storage): that storage can be lost while the memories are not, and then a locked person with memories would lose the download. The count is the fact itself.
- Guard: `the_lock_screen_offers_the_export_whenever_there_are_memories`. Driven in the browser harness: a new computer's lock screen (hidden), a returning signed-out computer and an ended trial (shown), a count that cannot be read (shown), Settings (shown).
- **Session 56: the guard tightened before its first run.** Planning the planted bugs showed it passed two of them: a `catch` that hid the download, and a looser test (`!info.memory_count`, which hides it on a reply without a count). It now pins the one assignment exactly, that `none` is set once, and that the `catch` sets nothing. Both plants then caught; so were the step count coming back and a Settings control outside its section. Independent code review of the walk-through's screens: nothing at BLOCKER, MAJOR or MINOR.

**3. The welcome names no number of steps** (founder: *"yes drop the number"*): *"A few short steps, about three minutes."* (two minutes without the sign-in step). The number it named had gone stale when ADR-105 L-e added the location step (the welcome said five, the next page "STEP 2 OF 6"). Guard: `the_welcome_hint_names_no_step_count`.

## 8.43 · ADR-104 / ADR-SEC-022 amendment 17 (2026-09-22, session 55) — §8.26 §6.4's allowlist widened by one: `startup_state` (FOUNDER-APPROVED)

The "Moving your memories" screen (ADR-105 amendment 1, L-f, `VAULT-KEY-AND-LOCATION.md`) asks the desktop how far a move has got before the entitlement guard exists: a start with a move waiting serves the page first and builds the guard after the move. So `startup_state` joins `guard::OPEN_COMMANDS`, the first entry outside §6.4's slots (account, export, erasure, logs, settings, maintenance status). Founder, shown what it answers and that it never touches a memory: *"yes all good"*. It is given the start's own progress and nothing else, answers only the stage, the move's phase, two byte counts and the folder being moved to, and takes no argument; the full reasoning is ADR-SEC-030 amendment 1. Pinned by `the_open_list_is_exactly_the_locked_allowlist` and `the_startup_command_is_given_nothing_but_the_start`.
