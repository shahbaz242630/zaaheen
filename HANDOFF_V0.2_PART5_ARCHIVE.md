# Zaaheen (Memory Vault) — Build Handoff, V0.2 Part 5 ARCHIVE (sessions 44–60, frozen 2026-09-24)

> **Frozen word for word at the end of session 60** (founder: *"lets archive it and start fresh clean handoff"*). The live file is `HANDOFF.md`. Everything below is as it stood.

**Current version:** V0.2 Closed Beta (BRD §6.2). **Last updated:** 2026-09-24, session 60.
...
## 1 · 🟢 Next — D4 (the desktop becomes a keeper client), folded into this batch; then the full gates, one installer and the live test

**Session 59 (2026-09-24): the live connection test found five problems, and all five are fixed in code, not yet committed** (on `feat/walkthrough-s58`, uncommitted, with session 58's walk-through screens). Founder: *"lets fix all of them .. and we dont run build until all are fixed"*; small crate-scoped tests only until then.

**Built, each with tests:**
- **rmcp 2.2.0 → 3.4.1 (ADR-SEC-032, in `CONNECT-APPS-DESIGN.md`), and `WIRE` 3 → 4** (the tool contract changed; a wire-3 keeper tells a new relay to restart). The updated Antigravity speaks the 2026-07-28 MCP spec and could not connect. vault-mcp **104 tests pass** (was 91), including Antigravity's exact sequence replayed against the relay and the keeper.
- **The read desk, cancelled calls dropped, the keeper started on connect, "one question at a time" in the tool descriptions (ADR-107, in `CONNECT-APPS-DESIGN.md`).** A burst of six reads from Cursor had turned slow into failed.
- **The Agents tab shows the apps actually connected** (session 36's design): the relay names its app to the keeper, the keeper keeps `.vault-clients.json`, the desktop's new `list_connected_apps` reads it. "Connect it for me" and the app picker now also in the Agents tab; exact steps for ChatGPT (Work/Codex), Antigravity (its real file path) and Claude Code (its documented command). Checked in the browser preview.
- **The health hints speak to people** (they told agents to "Run `vault-cli consolidate run`"; Claude said it to the founder twice). A guard test forbids developer words.
- **"Memories location"** heading, and **a bigger logo** (44 px on the full-page screens, 30 px in the top bar), both founder-asked.

**Do, in order:**
0. At open: `gh run list` (every workflow on `main`); disk (**15 GB free at close**: `target\debug\incremental` was cleared; the full wipe comes before the gates).

**Session 59's targeted tests, all green at close** (detached Task Scheduler runs, `tests-s59.ps1` + `tests-s59b.ps1`, through `gate-step.ps1`): vault-mcp **104/104** · vault-app keeper end-to-end **34/34** (after `WIRE` 3 → 4) · vault-app lib **461** (1 ignored) · vault-retrieval lib **107** · vault-tauri frontend contract **63/63** · clippy `--all-targets -D warnings` clean on vault-mcp, vault-app, vault-cli, vault-tauri, vault-retrieval · `cargo fmt --all` applied. Not run yet: the other crates' tests and the workspace build (the full gates, after D4).
1. **D4 — DESIGN LOCKED in session 60 (`DESKTOP-CLIENT-DESIGN.md`: ADR-108, ADR-SEC-033, an ADR-104 amendment); now building, tests first.** Two review rounds by two independent reviewers (round 1 NO-GO from both on the same blocker — a signed-out fresh install could never start; round 2 GO-WITH-FIXES, all applied). Founder approved every user-facing line, deleting the V0.1 bridge, and "Run now" pausing the AI apps. Iterations and the finding-by-finding table: `C:\Projects\MemoryVault-artifacts\d4-design\`. **Build phases, each tests-first, crate-scoped tests only:** P1 the `ADMIN` purpose (handshake) · P2 `InFlight` unconditional + `close()` + desk closed flag + drain · P3 `admin/` (moved op bodies, `AdminServer`, full and locked hosts, `MetadataStore::open_existing`) · P4 `dispatch_keeper` (engine cell, start-failure file, lock-mode fallback, `locked_no_key`, Starting heartbeat) · P5 `KeeperPool` ADMIN purpose + `ResolveError` · P6 `--take-over` / `--only-if-due` / runner audit row · P7 vault-tauri + frontend + source tests · P8 delete the V0.1 bridge. **Founder: no fresh build, wipe, gates or installer without asking first.**
   - **Session 60 progress: P1-P8 all WRITTEN, tests first** (uncommitted, on `feat/walkthrough-s58`). Keeper side under test in a detached run (`C:\Projects\MemoryVault-artifacts\tests-s60.ps1`, task `ZaaheenTestsS60`, status `gate-logs\s60-status.txt`): vault-mcp, vault-app lib + `keeper_end_to_end` + new `keeper_admin`, vault-cli bin — ONLY command shapes s57/s59 already ran (founder: *"make sure only effected parts are run"*). **Not run locally, CI only:** vault-storage alone (a new feature set could rebuild heavy deps). **Results (detached runs s60/s60b/s60c/s60d, all through `gate-step.ps1`):** vault-mcp **109/109** · vault-app lib **478** (1 ignored) · `keeper_end_to_end` **34/34** (AI apps' tool contract hash unchanged) · `keeper_admin` **15/15** (admin contract pinned: `f4d3f7fa…b2b6cf`) · vault-tauri lib **121/121** · `frontend_contract` **67/67** · clippy `--all-targets -D warnings` clean on vault-mcp, vault-app + vault-cli. **Deferred to the full gates (founder's go), both stopped when they began rebuilding heavy dependencies:** vault-cli's TESTS (never built locally since rmcp 3.4.1) and vault-tauri's CLIPPY (check graph never built since rmcp 3.4.1). Also CI-only: vault-storage's own tests. **Independent security review of the whole batch (s59 + D4): SAFE TO COMMIT**, no BLOCKER/MUST-FIX. All its points fixed the same session: S1 a locked keeper's Settings/audit open error now a stable code, detail to the log · S2 the gated-tool test loops over EVERY table-gated tool (+ a full-gate-reads-locked test) · S3 a maintenance run with no key does nothing (no fresh vault after "Delete everything") · N1 a recorded-but-missing folder counts as "memories may be there" · N2 the keeper resolves its models folder before the Starting heartbeat, publishing Failed on error · N3 the pool re-checks poison every resolve round. N4 logged below. **Re-tested in run s60e, ALL GREEN:** vault-app lib 478 (1 ignored) · `keeper_end_to_end` 34 · `keeper_admin` 16 · vault-tauri lib 121 · `frontend_contract` 67 · clippy vault-app + vault-cli clean. **Next:** the founder's go for the full gates — **disk is the constraint** (15 GB free; s57's cold wipe-and-gate run used ~87 GB).
   - **Noted for the founder:** the approved tidy-up line's long dash became a full stop (session 54's no-dash rule, pinned by `no_screen_shows_a_long_dash`).
   - **Tech debt from D4:** the full-host admin ops (`vault_app::admin::ops` over a real `Application`) have no local test — the old desktop placeholders were `#[ignore]` for the same reason; the catch-up run writes no audit row (only "Run now" does, from the runner); vault-app's `rusqlite` dev-dependency is kept (still used by `keychain/tests/live.rs`; dropping dev-deps changes feature resolution).
2. **Independent review of the security-relevant parts** of the whole batch (the gate's catch-all, the relay's cancel path, the clients file, D4) before the commit (memory `feedback_independent_review_before_security_commit`).
3. **The full gates, once** (fresh wipe + `gates-*.ps1` pattern, founder's go), then **one installer** (`release-build5.ps1`, the sandbox account settings), then the **live test with Claude (chat and Cowork), Cursor, ChatGPT (Work and Codex) and the updated Antigravity**: connect each, ask one question, check the Agents tab lists it, and a burst of questions answers or says busy instead of timing out. ADR-SEC-021's condition (a live test for rmcp 3.x) is met only by this.

**Lessons from session 59:**
- **A bare `cargo test` does not reuse the gate run's build:** `gate-step.ps1`'s environment (RUSTFLAGS `-D warnings`, line-tables-only, LIBCLANG, vcvars) IS the cache fingerprint. Two bare runs recompiled 600+ crates before I stopped them (memory updated).
- **Test the connection on the founder's real apps, one by one:** it found the new MCP spec (Antigravity), the queue failure (Cursor), and the developer-speak hints (Claude), none of which any test could have.
- **Memory is the live constraint:** Docker Desktop held 3.4 GB and doubled the rerank time; Claude Code reaped a background test run at low memory. Long runs go through Task Scheduler.
- **Slip, caught:** another `python -` heredoc; stopped at once, no harm.
...
- **The location (ADR-105 + ADR-SEC-030): designed and locked; L-a–L-f built (sessions 52–55), reviewed and plant-proved (session 56), gated and committed on `feat/s3-step4d` (session 57).** One record and one resolver for every process; the vault's own ID file; the folder checks; the move and its recovery; the screens (a setup step after sign-in, Settings "Move my memories…", the notice after a move) and "Start again in the default place". Session 55 added **amendment 1, L-f: the "Moving your memories" screen**; session 56 compiled, reviewed and plant-proved all of it. **Gated and committed in session 57; merged in session 58 (PR #77).**

### Recent sessions
- **59 (2026-09-24):** Website merged into `main` with publishing switched off (PR #78; `SITE_PUBLISH` unset; a code-scanning finding fixed tests-first, one dismissed as a false positive). **Live connection test on the sandbox installer, every app from a clean slate:** Cursor (install link) and Claude Desktop (extension; chat AND Cowork) connected and read correctly; ChatGPT desktop (Work and Codex, its own Add MCP Server form) too; the updated Antigravity could not connect (new MCP spec). It also found the queue failure, the developer-speak hints and the Agents-tab gaps. **All fixed in code, tests first, not yet committed** (§1): ADR-SEC-032, ADR-107, the connected-apps list, exact steps per app.
- **58 (2026-09-23):** S3 merged (PR #77 → `main` `21d5002`). Sandbox installer `Zaaheen_0.2.2_s3-4d-sandbox.msi` built (release-build5.ps1, the seven account settings). Walk-through of the setup pages in the browser preview: the recall-engine wait shown on the step that waits (it wrote to a hidden screen), clearer first-memory words, the tidy-up as short points, no "Replay welcome".
...
8. ~~**The agent-facing recovery hints name `vault-cli consolidate run`**~~ **Rewritten for people in session 59, with a guard test.**, a binary we don't ship (`vault-retrieval/src/structured_read_pipeline.rs` ~857–894). The user's path is the app's Consolidation → Run now.
...
12. **Ideas, not scheduled:** an email-handling agent (design before go-live, draft-for-approval first); rmcp 3.x: **done in session 59** (ADR-SEC-032); its live test is §1 step 2.

---

## 4 · 🐛 Tech debt (live, with anchors)

- **The connected-apps file's write retry sleeps on a runtime thread** (`vault-app/src/keeper/clients.rs` ~300-308): `std::thread::sleep` inside the async task while holding the `Clients` mutex, bounded (~80 ms). Move to `spawn_blocking` or a tokio sleep (session 60 security review N4).

- **The declared MSRV is stale:** `Cargo.toml` says `rust-version = "1.81"`, but rmcp is edition 2024 (since 2.x) and 3.4.1 declares 1.88; the toolchain is pinned at 1.92. Raising it will turn on clippy lints held back at 1.81 (`map_or(true, …)` → `is_none_or`, session 57). Its own small change, with a clippy run (session 59).
